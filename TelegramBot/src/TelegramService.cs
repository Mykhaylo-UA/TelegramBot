...
namespace TelegramBot
{
    public class TelegramService : IChatService, IDisposable
    {
            ...
            ILogger<TelegramService> logger)
        {
            ...
            _botClient = new TelegramBotClient("1899289077:AAHetQSleEYTsH5c9uh0Q7uB-tQOU-vvyBc");
        ...
        {
            _logger.LogTrace("Message received from '{Username}': '{Message}'",
                e.Message.From.Username ?? e.Message.From.FirstName, e.Message.Text);
        ...
        {
            _logger.LogTrace("Callback received from '{Username}': '{Message}'",
                e.CallbackQuery.From.Username ?? e.CallbackQuery.From.FirstName, e.CallbackQuery.Data);

            if (sender is TelegramBotClient client)
            {
                // This removes the keyboard, but we could also update one here...
                await client.EditMessageReplyMarkupAsync(e.CallbackQuery.Message.Chat.Id,
                    e.CallbackQuery.Message.MessageId, null);
        ...
        public async Task<bool> UpdateMessage(long chatId, int messageId, string newText,
            Dictionary<string, string> buttons = null)
        {
            try
            {
                ...
                await _botClient.EditMessageTextAsync(new ChatId(chatId), messageId, newText,
                    parseMode: ParseMode.MarkdownV2, replyMarkup: GetInlineKeyboard(buttons));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> SendMessage(long chatId, string message, Dictionary<string, string> buttons = null)
        {
            try
            {
                ...
                await _botClient.SendTextMessageAsync(new ChatId(chatId), message, parseMode: ParseMode.MarkdownV2,
        ...
        // It can will be delete
        public async Task<bool> SendInvoice(long chatId)
        {
            try
            {
                await _botClient.SendInvoiceAsync((int)chatId, "Product Title", "Description Product", "payload Product", "632593626:TEST:sandbox_i461953347",
                    "start_parameter", "UAH", 
                    new List<LabeledPrice>()
                    {
                        new LabeledPrice(){Amount = 2800, Label = "28"}
                    });
                return true;
            }
            catch(Exception ex)
            {
                _logger.LogError(ex, "Error while sending invoice");
                return false;
            }
        }
